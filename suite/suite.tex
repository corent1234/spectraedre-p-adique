...
\title{Follow up on paper \cite{cornou}}
\author{}
...
\section{On the geometry of solutions to LP}

In \cite[Def.\,3.1]{cornou} we define polyhedra in $K^n$, for a valued field $(K, \val)$, as the
...
We say that a form $m$ vanish on $x$ if $\val(m(x))=+\infty$.
...
\begin{assumption}
There exist an element $x \in \PP$ and an index $i \in \{1,\ldots,d\}$ such that
$\val(\ell_i(x)) < +\infty$, that is, the forms $\ell_1,\ldots,\ell_d$
do not vanish identically on $\PP$.
\end{assumption}

This excludes the trivial case of $\PP$ being an affine subset of $K^n$.
Nevertheless the linear forms $\ell_i$ might vanish at some
$x \in \PP$ ({\it i.e.} one might have $\val(\ell_i(x)) = +\infty$ for some $x$).
...
\begin{definition}
  ...
  $$
  Its \emph{rank} is $d-\mathrm{corank}(x)$.
\end{definition}
Rank and corank of $x \in\PP$ depend on the description \eqref{eq_def_polyhedra}.


\section{Semidefinite-representable sets over $K=\QQ_p$}
