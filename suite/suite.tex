...
\begin{assumption}
For every $x \in \PP$, there exist $i = 1,\ldots,d$, such that
$\val(\ell_i(x)) < +\infty$.
\end{assumption}
This corresponds to the situation over $K=\R$ when $\PP$ is not a simple
affine set.


Some of the linear forms $\ell_i$ might vanish at some $x \in \PP$
({\it i.e.} one might have $\val(\ell_i(x)) = +\infty$). 
