...
The basic closed semialgebraic set $\mathcal{R} = \{(x_1,x_2) \in \R^2 : 1-x_1^4-x_2^4 \geq 0\}$ is not a spectrahedron but it is semidefinite representable with representation:
$$
%%\mathcal{R} =
\left\{\begin{bmatrix} x_1 \\ x_2 \end{bmatrix} \in \R^2 :
\exists\,
...
Let $f : K^n \to K^n$ be a linear map and $\PP$ a polyhedron in $K^n$ defined only by inequalities. We assume that the matrix of $f$ in the canonical basis of $K^n$ is diagonal.
...
Let $f : K^n \to K^{n-1}, \: (x_1,\dots,x_{n-1},x_n) \mapsto (x_1,\dots,x_{n-1})$ and $\PP$ a polyhedron in $K^n$ defined only by inequalities. 
