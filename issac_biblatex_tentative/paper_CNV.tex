...
\end{proposition}
\begin{proof}
Let $P \in K^m$. One can see by immediate change of variables that $\Qc$ is a polyhedron in $K^m$ if and only if
$\Qc+P$ is a polyhedron in $K^m$.
...
Here, $f : K^n \to K^m$ is an affine map and $\PP$ a polyhedron in $K^n.$
Let us write $\PP = \PP_I \cap \PP_E$
with $\PP_I \coloneq \left\lbrace X \in K^n,  AX+B \geq 0 \right\rbrace$
and $\PP_E \coloneq \left\lbrace X \in K^n,  CX+D=0 \right\rbrace = P + \overrightarrow{\PP_E}.$ 
Then $f(\PP) = f_{\PP_E}(\PP_I).$

By a translation of $-P$ in $K^n$, we can assume that
$\PP_E$ is a linear vector space: we are reduced to proving that, if $\tilde{f}: \PP_E \to K^m$ is an affine map and $\PP_I$ is a polyhedron in the vector space $\PP_E$ defined only by inequalities, then  $\tilde{f}(\PP_I)$ is a polyhedron.
...
\begin{proposition}[Characterization of polyhedra] \label{prop:carac}
    Non-empty polyhedra are image of polydiscs under a linear mapping.
\end{proposition}
\begin{proof}
