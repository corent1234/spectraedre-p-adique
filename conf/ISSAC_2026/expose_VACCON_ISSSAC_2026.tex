...
\date{\pgfuseimage{logo-ul} \hspace{1cm} \pgfuseimage{cnrs} \hspace{1cm}  \pgfuseimage{xlim}  \hspace{1mm}  \vspace{2mm} \\ Séminaire Limougeaud de Calcul Formel, June 18th, 2026}
...
{\bf\color{blue} Fact:} Polyhedra resp. Spectrahedra are {\bf convex}.
%as affine pre-images of resp.
%\begin{equation*}
%  \begin{aligned}
...
%
%{\bf\color{blue} Fact:}
%They are {\bf basic semi-algebraic} sets: $A(x) \succeq 0$ if and only if all the principal minors of $A(x)$ are $\geq 0.$
...
\begin{figure}
  ...
  \caption*{\hfill The convex hull of the Fermat's quartic $x^4+y^4=1$ \\
    \hfill is a spectraedral shadow but not a spectrahedron}
\end{figure}
\pause

Not every convex (compact) semialgebraic set is a spectrahedral shadow (Scheiderer, 2018).
...
\begin{expl}
\begin{enumerate}
\item $\QQ_p$, $p$-adic valuation, $\ZZ_p$, $\kappa = \FF_p$, $\pi = p.$ \pause
\item $\FF_q((t))$, $t$-adic valuation, $\FF_q[[t]]$, $\kappa = \FF_q$, $\pi = t.$ \pause
\item $\QQ((t))$, $t$-adic valuation, $\QQ[[t]]$, $\kappa = \QQ$, $\pi = t.$ \pause
\item their finite extensions. \pause
\end{enumerate}
\end{expl}
\pause
...
%
%\begin{rmk}
%Discoids play a central role in the family of OM factorization algorithms. \pause
%See the next talk for some application to the clusterization of the roots of polynomials over $K.$
%\end{rmk}
%
%
%\end{frame}
...
\pause
\begin{expl}
\[ D(c, \alpha) = \left\lbrace x \in K^n : val \left( x-c  \right) \geq \alpha= \right\rbrace= \left\lbrace x \in K^n : val \left( \pi^{-\alpha} (x-c)  \right) \geq 0 \right\rbrace.\]
...
\begin{definition}
  We say that a matrix $M \in K^{d \times d}$ is {positive semidefinite} ($M \succeq 0$) \pause
...
\begin{definition}
  ...
  A \emph{spectrahedron in $K^{d\times d}$} \pause is a set of the form
  $$
...
\begin{definition}
  ...
  A \emph{spectrahedron in $K^n$} \pause is any affine section of a set of the form
  $$
...
%  for some $d, e \in \mathbb{N}$. \pause
%  
...
Basically, same idea as over $\mathbb{R}$: we can put the inequalities of valuation into some big diagonal matrix 
to get some linear matrix inequality.
\end{proof}

\end{frame}

\begin{frame}
\frametitle{Some spectrahedron are not polyhedron} \pause
\begin{example}
...
\end{example} \pause
\begin{proof} \pause
  By characterization of positive semi-definite matrices, one has \pause $\mathcal{S} = \{x \in K : \val(x(x + \pi^{-1})) \geq 0\} =
  \{x \in K: \val x \ge 1\} \cup \{x \in K: \val (x - \pi^{-1}) \ge 1\}$ \pause which is not a ball in $K$. \pause
...
\begin{prop}
Spectrahedra of $K$  are finite union of balls. 
\end{prop} \pause
\begin{proof}
If $S$ is a spectrahedra of $K$ not reduced to a point, then \pause
\begin{align*}
S &= \{x \in K : A_0+x A_1 \succeq 0\} \\
  &= \cap_{i=0}^{d-1} \{x \in K : \val(a_i(x)) \geq 0\}
\end{align*} \pause
for some integer $d$, matrices $A_0$ and $A_1$ in $K^{d \times d}$, and $\chi_{A_0+xA_1}(T) = \sum_{i=0}^{d} a_i T^i.$ \pause
%The intersection
%of two diskoids is either empty or a diskoid. \pause
%Consequently, spectrahedra of $K$ are diskoids.

The set $ \{x \in K : \val(a_i(x)) \geq 0\}$ is a \textbf{diskoid} (see the next talk for more on diskoids). \pause

Any diskoid is a finite union of balls. \pause

Any intersection of finite union of balls is a finite union of balls. \pause
\end{proof}

\begin{block}{Question}
Are all spectrahedron diskoids?
\end{block}

\end{frame}


\subsection{Spectrahedral shadows and polyannuli}
...
\pause
  ...
  Therefore $\val\left(x\right)=-\val\left(y\right)\le -(-b) = b$ and $x \in \mathcal{C}_K(0,a,b)$. \pause
...
\end{thm} \pause
\begin{proof}
Spectrahedron of $K$ are finite union of balls. \pause

If the residue field of $K$ is infinite, no annulus
can be finite union of balls. \pause

For example with the unit circle:
\[ \{x \in K \,|\,  \val(x) =0\}=\amalg_{l \in \kappa} D(b_l, \val (\pi))\] \pause for some representatives $b_l \in O_K$ of the classes of ($O_K \setminus \lbrace 0 \rbrace) / \pi O_K = \kappa^\times.$
\end{proof}
\end{frame}
...
\begin{block}{Summary}
\begin{itemize}
\item Definition and first properties of polyhedron and spectrahedron over $K$. \pause
...
\item Can we give an explicit representation?
\end{itemize}
\end{block}
\end{frame}
