...
\begin{prop}
Spectrahedra of $K$ \pause are diskoids. 
\end{prop} \pause
\begin{proof}
...
\end{align*} \pause
for some integer $d$, matrices $A_0$ and $A_1$ in $K^{d \times d}$, and $a_i$ the coefficient of the corresponding characteristic polynomial. \pause
The intersection
...
\end{thm} \pause
\begin{proof}
Spectrahedron of $K$ are diskoids and thus are finite union of balls. \pause

If the residue field of $K$ is infinite, no annulus
can be finite union of balls. \pause

For example: 
\[ \{x \in K \,|\,  \val(x) =0\}=\amalg_{l \in \kappa} B(b_l, \vert \pi \vert)\] \pause for some representatives $b_l \in O_K$ of the classes of $O_K / \pi O_K = \kappa.$ \pause
