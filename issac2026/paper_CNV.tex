...
\begin{abstract}
  ...
  their images through linear maps, are respectively the feasible sets
  ...
  For $K$-polyhedra and linear programming over $K$ we give an algorithm
  based on computation of Smith normal forms.
  We prove that basic properties of semidefinite-representable sets generalize
  to the valued setting.
  In particular, we showcase examples of non-polyhedral $K$-spectrahedra and
  of sets that are semidefinite-representable over $K$ but are not $K$-spectrahedra.
\end{abstract}
...
Real polyhedra and linear programming (LP) are at
the heart of convex geometry and optimization, inasmuch as several
optimization problems can be cast as, or relaxed to LP.
An important generalization of LP is semidefinite programming (SDP),
namely the problem of minimizing a linear function over the set of
positive semidefinite affine combinations of a finite collection of
real symmetric matrices.
The latter problem has interested the
community of real algebra and polynomial optimization,
mostly because of the relaxation method known as moment-SOS hierarchy
\cite{henrion2020moment}.

Several properties of polyhedra generalize to spectrahedra, the feasible
...
(called LMI, linear matrix inequality).
Nevertheless spectrahedra are not stable under linear image, {\it i.e.},
projections of spectrahedra (called semidefinite-representable sets)
are in general not spectrahedra. It has been asked by Nemirovski
\cite{nemirovski2007advances} whether every convex semialgebraic set admits
a semidefinite-representation, {\it i.e.} is the projection of a spectrahedron,
then conjectured by Helton and Nie \cite{helton_sufficient_2008};
this has been disproved by Scheiderer that exhibited compact counterexamples
\cite{scheiderer_spectrahedral_2017}.

Another difference with polyhedra is that the base field where the LMI is defined
might not be sufficient to describe points on the corresponding spectrahedron.
Spectrahedra defined over $\QQ$ might not admit rational points, hence the
solution of a SDP problem defined over $\QQ$ might not necessarily be
expressible as rational combination of input data.
However it is still algebraic over the base field, hence,
assuming the input data of a SDP belongs to an effective field $K$,
it follows that a solution can be described and computed as
element of a vector space over some algebraic extension of $K$.
This leads to interesting algorithmic questions.
Complexity bounds for the feasibility of SDP have been obtained in
\cite{porkolab1997complexity} and are based on general quantifier
elimination bounds \cite{renegar1992computational}. Explicit exact
algorithms exploiting the underlying determinantal structure of SDP
have been designed more recently \cite{henrion2016exact,NALDI2018206}.

%% j'ai commenté ça car trop d'information pour quelque chose dont on parle pas dans le reste du papier
%%
%%
%It is worth mentioning that the complexity status of SDP is widely open.
%Ramana's dual \cite{ramana1997exact} implies a series of general
%results: in the Blum-Shub-Smale model \cite{blum1989theory}, the feasibility
%of SDP is in $\text{NP} \cap \text{co-NP}$, whereas it is not known to be
%in NP in the Turing model; checking boundedness, attainability and optimality
%can be polynomially reduced to the feasibility of SDP \cite{ramana1997exact}.
%%% CE PARAGRAPHE ME PARAIT MOINS PRIORITAIRE
...
Nevertheless the complexity status of SDP, unlike LP, remains widely
open. This has motivated the development of variants of this problem beyond the real case.
Tropical convexity and in particular generalizations of polyhedra and spectrahedra to the tropical setting
have been proposed in \cite{develin2004tropical,allamigeon_tropical_2020} and tropical methods applied to
non-archimedean SDP \cite{allamigeon2016solving}. It is worth mentioning that tropical spectrahedra satisfy
the Helton-Nie property \cite{allamigeon2019tropical}. Linear matrix inequalities with parametric coefficients
have been studied in \cite{naldi2025solving}.

\paragraph{Main contributions.}
In this paper we consider the situation where input data are defined over $(K,\val)$, where $K$ is a field and
$\val$ a valuation map on $K$.
The prototype of valued field we target is the field $K=\QQ_p$ of $p$-adic numbers equipped with
$p$-adic valuation. A polyhedron over a valued field is naturally defined as a set of vectors such that some
linear forms have nonnegative or infinite valuation ({\it cf.} \Cref{def_polyhedra}). Our main result in this
...
Next we focus on linear programming. We describe in \Cref{sec:LP}
an algorithm based on computation of Smith Normal Forms for solving LP problems with
data over $(K,\val)$, and prove its correctness (\Cref{algo_LP}).

In \Cref{sec:spectrahedra} we define spectrahedra and semidefinite-re\-pre\-sen\-ta\-ble sets
over $(K,\val)$. A spectrahedron over a valued field is defined as the set of matrices
in some affine space that are positive semidefinite, {\it i.e.} all of whose eigenvalues
have nonnegative valuation in some algebraic closure of $K$. This defines, as in the
real case, a class of semialgebraic sets including polyhedra.

Nevertheless and in contrast with the real case, we exhibit a class of subsets of $K$
(thus in dimension one) that are semidefinite-representable but not spectrahedral, under mild assumptions:
...
Throughout the paper, $(K,\val)$ refers to a complete valued field, {\it i.e.} $K$ is a field
and $\val : K \twoheadrightarrow \Gamma \cup \{+\infty\}$ is a valuation on $K$, $\Gamma$ an
ordered abelian group. The valuation is often assumed to be discrete ($\Gamma=\ZZ$)
with uniformizer $\pi \in K$. The valuation ring of $K$ is
$$
\OK = \{x \in K : \val(x) \geq 0\},
$$
and $\kappa = \OK/\mathfrak{m}_K$ its residue field, with
...
Let us now briefly give an overview of classical, {\it i.e.} real, polyhedra and spectrahedra,
...
matrices. A matrix $M \in \sym_d(\R)$ is {positive semidefinite} ($M \succeq 0$) whenever the quadratic form $x \mapsto x^TMx$ is nonnegative for all $x\in \R^n$, that is, if and only if the eigenvalues of $M$ are nonnegative. The set $\sym_d^+(\R) \subset \sym_d(\R)$ of positive semidefinite matrices is a closed convex cone with non-empty interior in $\sym_d(\R)$. It is also a semialgebraic set inasmuch as $M \succeq 0$ if and only if its principal minors are nonnegative.
...
a {real spectrahedron}. As (linear preimage of) affine sections of $\sym_d^+(\R)$, real spectrahedra
are closed convex semialgebraic sets, possibly with empty interior.
%%Moreover, they are basic semialgebraic sets, indeed they are defined by sign conditions on the principal minors of the defining matrix as well as sign conditions on the coefficients of its characteristic polynomial (cf. \Cref{ell3}).
...
%  \label{SDP}
%\begin{array}{rcll}
%  p^* & \coloneq & \inf_{x \in \R^n} & \left\langle c, x \right\rangle \\
...
$$
When $m>0$, the set is also called a {spectrahedral shadow}. There are examples of
spectrahedral shadows that are not spectrahedra (cf. Ex. \ref{fermat_quartic}) and of compact
...
In this section, we define polyhedra over a \simone{complete discrete}\footnote{\simone{On utilise vraiment le ``complete'' et ``discrete'' ici ? en tous cas pas pour la définition. Peut être dire (après la définition) qu'on fait cette hypothèse même si certains résultats sont vrais pour une valuation plus générale ?}} valued field $K$ with valuation $\val$ and a uniformizer $\pi$, and we prove that the class of polyhedra is closed under linear transformations, as in the real case. 
