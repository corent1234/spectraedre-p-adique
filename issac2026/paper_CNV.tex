...
\begin{theorem}[direct image (DI)] 
Let $f : K^n \to K^m$ be an affine map and $\PP$ a polyhedron in $K^n.$
Then $f(\PP)$ is a polyhedron in $K^m$. \label{theo:DI}
\end{theorem}

Next we focus on linear programming over valued fields. We describe in \Cref{sec:LP}
an algorithm for solving LP problems with data over a valued field $(K,\val)$
and prove its correctness. \Cref{algo_LP} reduces the input program to a diagonal LP 
by applying Smith Normal Form.

In \Cref{sec:spectrahedra} we define spectrahedra and semidefinite representable sets
over a valued field $K$.
