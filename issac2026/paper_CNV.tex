...
\Cref{def_polyhedra}) : let $A \in K^{d \times n}$, $B,D \in K^n$, and
$C \in K^{e \times n}$, be the matrices such that $Ax+B = (\ell_1,\ldots,\ell_d)^T$
...
\begin{proof}
  ...
  \[\Qc=\left\lbrace y \in K^n,  AU^{-1} y+B \geq 0, CU^{-1} y+D=0 \right\rbrace.\]
