...
Below we use\footnote{rajouté notation $\succeq$ et modifié dans le texte}
the following notation for $v=(v_1,\ldots,v_n) \in K^n$:
$$
v \succeq 0
\,\,\,\,
\xleftrightarrow{\text{def}}
\,\,\,\,
\forall i=1,\ldots,n, \, \val(v_i)\geq 0.
$$
yielding the following matrix form for $\PP$ (according to notation of
\Cref{def_polyhedra}):
\begin{equation}\label{eq_def_polyhedra_compact}
  \PP = \left\lbrace x \in K^n : Ax+B \succeq 0, Cx+D = 0 \right\rbrace,
\end{equation}
for $A \in K^{d \times n}$, $B \in K^d$, $C \in K^{e \times n}$, and
$D \in K^e$ such that $Ax+B = (\ell_1,\ldots,\ell_d)^T$
and $Cx+D = (m_1,\ldots,m_e)^T$.
...
Moreover by definition the class of polyhedra is closed under intersection. Without loss of generality\footnote{je ne
sais plus si ça est nécessaire, si ce ne l'est pas, je l'enleverais.}, one may
...
We often fix a linear basis $e_1,\ldots,e_n$ of $K^n$ which we call canonical\footnote{j'ai rajouté ça}.
...
\begin{proof}
  ...
  \[\Qc=\left\lbrace y \in K^n,  AU^{-1} y+B \succeq 0, CU^{-1} y+D=0 \right\rbrace.\]
...
\begin{proof}
  ...
  matrix form: $\PP = \{x \in K^n : Ax+B \succeq 0\}$, for some 
  $A \in K^{d \times n}$, $B \in K^d$,
...
defined by the following linear matrix inequality (for some $B' \in K^d$):
\[
Q_A A P_A^{-1}
%\begin{bmatrix}
%\delta_1	& 		& 			&   &		 &  &a_1	\\
...
$s \in \llbracket r+1, d \rrbracket$
...
1	& 		& 			&   &		 &  &a_1	\\
			...
			&		&			&	&		 & 0&a_{n-1} \\
			&		&			&	&		 &	&1 	\\
\end{bmatrix} \begin{bmatrix} x_1 \\ \\  \\ \\ \vdots \\  \\ \\ x_n \\ \end{bmatrix} + \begin{bmatrix} b_1 \\ \\ \\  \\ \vdots  \\ \\ \\ b_n \\ \end{bmatrix}
\succeq 0.
\] %%ARRIVATO QUI
...
$\val(x_i-a_ib_n+b_i) \geq 0$. Then the previous computations prove that $f(\PP')\subset \Qc.$
...
$f(\PP')$ as a polyhedron\footnote{$f(\PP')$ ou bien $f(\PP)$ ?}.
\end{proof}
...
\begin{proof}
  ...
  Consequently, translating by $-f(O)$ in the codomain\footnote{Qu'est-ce qu'est $O$ ? C'est $v$ ?},
  one can reduce to the case of Proposition \ref{prop:DIl}.
\end{proof}

We conclude with the proof of the main result of this section.

\begin{proof}[Proof of Theorem \ref{theo:DI}]
...
This is the case thanks to Proposition \ref{prop:DIi}
...
A \emph{linear programming problem} (or \emph{linear program}) \emph{with data $(A,b,c,\allowbreak D,e)$} is defined
as:
\begin{equation}
  \tag{LP}\label{LP}
\begin{array}{rcll}
