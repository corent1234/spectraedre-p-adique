...
In this paper we consider the situation where input data are defined over fields $K$ with a prescribed valuation map $\val$ on $K$. The prototype of valued field we target is the field $\QQ_p$ of $p$-adic numbers equipped with $p$-adic valuation. A polyhedron over a valued field is naturally defined as a set of vectors such that some linear forms have nonnegative or infinite valuation ({\it cf.} \Cref{def_polyhedra}). Our main result in this context is the following theorem proved in \Cref{ssec:projections}:
...
Next we focus on linear programming over valued fields. We describe in \Cref{sec:LP}
an algorithm based on computation of Smith normal forms for solving LP problems with
data over a valued field $(K,\val)$, and prove its correctness (\Cref{algo_LP}).

In \Cref{sec:spectrahedra} we define spectrahedra and semidefinite-representable sets
