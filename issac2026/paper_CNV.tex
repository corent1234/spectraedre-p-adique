...
a {real spectrahedron}. As a linear preimage of an affine section of $\sym_d^+(\R)$, real spectrahedra
...
\begin{example}
  ...
  $\ell_i(x)=x_i$, $i=1,\ldots, n$, and $e=0$, {\it i.e.} $\OK^n = \{x \in K^n : x \succeq 0\}$.
\end{example}

\subsection{Projections and direct image of polyhedra}
\label{ssec:projections}
The goal of this section is to prove \Cref{theo:DI}, {\it i.e.}, that
...
We often fix a linear basis $e_1,\ldots,e_n$ of $K^n$ which we call canonical.
...
%\end{bmatrix},
%\]
with $Q_A \in \GL_d(\OK)$ and $P_A \in \GL_n(\OK)$ such that $P_A e_n = e_n$,
