...
method known as moment-SOS (or Lasserre) hierarchy \cite{henrion2020moment}.

Most of the properties of polyhedra generalize to spectrahedra, the feasible
sets of SDP ({\it e.g.} convexity). Nevertheless 

The main motivation for studying the effective aspects of SDP is the fact
that the complexity status of SDP is, contrary to the LP case, widely open.
Ramana's construction \cite{ramana1997exact} implies a series of general
...
An element of $\OK$ is called integral (at\footnote{Je comprends pas, c'est bien integral
à la valuation?} $\val$).
...
size $p \times q$ by ${\bf 0}_{p,q}$, and the $t \times t$ identity matrix by $I_t$.
