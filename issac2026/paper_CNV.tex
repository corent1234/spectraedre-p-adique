...
Consequently, $x_1-a_1 v'_n +b_1=O(1)+O(\pi^{\val(a_1)})=O(1).$
Similarly, for all $i \in \llbracket 2,r \rrbracket,$
$x_i-a_i v'_n+v'_i=O(1)+O(\pi^{\val(a_i)})=O(1).$

We define the polyhedron $\Qc \subset K^{n-1}$ by the following inequalities:
 for all $i \in \llbracket 1,r \rrbracket,$
$\val(x_i-a_iv'_n+v'_i) \geq 0$. Then the previous computations prove that $f(\PP')\subset \Qc.$
