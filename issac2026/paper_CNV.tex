...
\begin{proposition}\label{main_2}
If the residue field of $K$ is infinite, then no (non-trivial) annulus of $K$ can be a spectrahedron of $K.$
\end{proposition}
...
spectrahedral shadows that are not spectrahedra (cf. \Cref{fermat_quartic}) and of compact
...
Let us assume it is the one defined for $s = n.$
The only condition on $x_n$
is then $\val(a_nx_n + v'_n) \geq 0.$
...
Then let $x_n \coloneq -v'_n.$
We check that for $i \in \llbracket 1,n-1 \rrbracket,$
$x_i+a_ix_n+v'_i=x_i-a_iv'_n+v'_i=O(1),$
and $x_n+v'_n=O(1)$. Hence $(x_1,\dots,x_n) \in \PP_2$
...
Then $x_n=-v'_n+O(1)$
and $x_1+a_1 x_n+v'_1=O(1).$
Consequently, $x_1-a_1 v'_n +v'_1=O(1)+O(\pi^{\val(a_1)})=O(\pi^{\val(a_1)}).$
Thus $a_1^{-1}x_1- v'_n +a_1^{-1}v'_1=O(1)$.
...
We define the polyhedron $\Qc \subset K^{n-1}$ by the following inequalities:
$\val(a_1^{-1}x_1- v'_n +a_1^{-1}v'_1) \geq 0$ and for all $i \in \llbracket 2,n-1 \rrbracket,$
...
Finally, 
$x_n+v'_n=-a_1^{-1}(x_1+v'_1)+v'_n=O(1)$ thanks to the inequality satisfied by $x_1.$
Hence $(x_1,\dots,x_n) \in \PP'$
which means that $(x_1,\dots,x_{n-1}) \in f(\PP').$
Thus $f(\PP')= \Qc$ and $f(\PP')$ is a polyhedron.

Therefore, $f(\PP)$ is a polyhedron, and inequalities are enough to define
...
%    Let $\PP = \{x \in K^n : Ax+v \succeq 0, Bx = w\}$ be a polyhedron in $K^n$.
    ...
    Thus $\PP = x_0+JP\mathcal{Q}$ is empty or a polydisc.
% $\PP' = P $.
...
for some polynomial $f \in K[x]$ and some real number $r \geq 0.$
\end{definition}
