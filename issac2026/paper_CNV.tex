...
and $\val : K \twoheadrightarrow \Gamma \cup \{+\infty\}$ is a valuation on $K$, where $\Gamma$ 
is an ordered abelian group. The valuation is often assumed to be discrete ($\Gamma=\ZZ$)
...
In this section, we define polyhedra over a complete discrete valued field $K$ with valuation $\val$ and a uniformizer $\pi$, and we prove that the class of polyhedra is closed under linear transformations, as in the real case. 
