...
real symmetric matrices. The latter problem has interested the
...
projection of spectrahedra (called semidefinite representable sets)
...
The differences in the geometry of polyhedra versus spectrahedra is also reflected
on different behaviors of LP and SDP. Spectrahedra defined over $\QQ$
might not admit rational points, hence the solution of a SDP problem
defined over $\QQ$ might not necessarily be expressible as rational combination
of input data. However it is still algebraic over the base field, hence,
assuming the input data of a SDP belongs to an effective field $K$
then a solution can be described and computed as
element of a vector space over some algebraic extension of $K$.
%%% j'ai deplacé ça ici
%%
%%
General complexity bounds for the feasibility of SDP are based on general
quantifier elimination \cite{ramana1997exact,porkolab1997complexity}.
There has been
in the recent past the development of exact algorithms for
deciding whether a spectrahedron is empty, {\it i.e.} to solve a linear matrix
inequality.
 or on
explicit algorithms exploiting the underlying determinantal structure of SDP
\cite{henrion2016exact,NALDI2018206}.
...
Generalizations of spectrahedra and SDP have been proposed in the context of tropical geometry \cite{iriarte2022polyhedral} and tropical methods applied to non-archimedean semidefinite programming \cite{allamigeon2016solving,allamigeon_tropical_2020}. It is worth mentioning that tropical spectrahedra satisfy the Helton-Nie property \cite{allamigeon2019tropical}.

\paragraph{Main contributions.}
In this paper we consider the situation where input data are defined over fields $K$ with a prescribed valuation map $\val$ on $K$. The prototype of valued field we target is the field of $p$-adic numbers with $p$-adic valuation. A polyhedron over a valued field is naturally defined as a set of vectors such that some linear forms have nonnegative or infinite valuation ({\it cf.} \Cref{def_polyhedra}). Our main result in this context is the following theorem proved in \Cref{ssec:projections}:
