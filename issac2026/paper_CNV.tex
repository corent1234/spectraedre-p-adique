...
\begin{proof}
  ...
  assume that $\PP \neq \emptyset$ and that it is defined as
$$
\PP = \left\lbrace x \in K^n : \val (l_i (x)) \geq 0, \textrm{ for } i \in I\right\rbrace
$$
for some finite set $I \subset \N$ and $l_i \in K[x]_1$, for $i \in I$ .

Thanks to Definition \ref{smith_nf}, (the matrix of) $f$ can be written $f = Q^{-1} \Delta P$
with $P \in \GL_n(\OK),$ $Q \in \GL_m(\OK)$
and
$$
\Delta = \diag(\lambda_1, \ldots, \lambda_r, {\bf 0}_{\min(m,n)-r}) = \mathrm{SNF}(f)\in K^{m \times n}
$$
...
with $\PP_I \coloneq \left\lbrace x \in K^n,  Ax+B \geq 0 \right\rbrace$
and $\PP_E \coloneq \left\lbrace x \in K^n,  Cx+D=0 \right\rbrace = v + V$,
for some vector space $V$.
%%\overrightarrow{\PP_E}
Then\footnote{Est-ce que $f_{\PP_E}$ c'est la restriction ? Mais donc c'est $=\tilde{f}$ ?}
$f(\PP) = f_{\PP_E}(\PP_I).$

By a translation of $-v$ in $K^n$, we can assume that
$\PP_E$ is a linear vector space: we are reduced to proving that, if
$\tilde{f}: \PP_E \to K^m$ is an affine map and $\PP_I$ is a polyhedron in the vector space $\PP_E$ defined only by inequalities, then  $\tilde{f}(\PP_I)$ is a polyhedron.
This is the case thanks to \Cref{prop:DIi}
...
\end{proposition}
\begin{proof}
    Let $\PP$ be a polyhedron in $K^n$. Then $\PP = \{x \in K^n : Ax+B \ge 0, Dx = e\}$ for some $A \in K^{d \times n}, b \in K ^d,  D\in  K^{m\times n}, e \in K^m$.

    If $\PP$ is nonempty then there exists $x_0 \in K^n$ such that $Dx_0=e$. Let $J \in K^{n \times R}$ such that $\textrm{Im} J = \ker D$ where $R$ denotes the dimension of $\ker D$. Then $\PP = x_0 + J\PP'$ where $A' \coloneq AJ$ $b' \coloneq Ax_0 + b$ and
    $\PP' \coloneq \{y \in K ^R: A'y + b' \ge 0 \}$.
    ...
    Then $\PP' = \{y \in K^R : QSP^{-1}y + b' \ge  0\} = P\{z \in K^R : Sz + Q^{-1}b' \ge 0 \}$.
    Let $b'' \coloneq Q^{-1}b$ then $P' = P \{z \in K^R : s_i z_i + b''_i \ge 0, i= 1\ldots R\}$. The set $\{z \in K^R : s_i z_i + b_i \ge 0\}$ is either empty or a polydisc (possibly with infinite radii) and $\PP = x_0 + JP \{z \in K^R: s_i z_i + b_i \ge  0, i=1,\ldots,R\}$ which concludes the proof.
\end{proof}
