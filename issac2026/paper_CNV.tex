...
Let us now give a short introduction to classical ({\it i.e.} real) polyhedra and
spectrahedra.
Let $\sym_d(\R) \subset \R^{d\times d}$ be the vector space of $d \times d$ real symmetric
matrices. A matrix $M \in \sym_d(\R)$ is \emph{positive semidefinite} ($M \succeq 0$) whenever the quadratic form $x \mapsto x^TMx$ is nonnegative for all $x\in \R^n$, that is, if and only if the eigenvalues of $M$ are nonnegative. The set $\sym_d^+(\R) \subset \sym_d(\R)$ of positive semidefinite matrices is a closed convex cone with non-empty interior in $\sym_d(\R)$. It is also a semialgebraic set inasmuch as $M \succeq 0$ if and only if its principal minors are nonnegative.
...
$$
When $m>0$, the set is also called a \emph{spectrahedral shadow}. There are examples of
...
\label{sec:LP}
A \emph{linear programming problem} (or \emph{linear program})
\emph{with data $(A,b,c,\allowbreak D,e)$} is defined as:
...
A vector $x \in K^n$ is called \emph{feasible} for~\eqref{LP} if $x \in \PP$. A feasible vector $x^* \in \PP$ is called a \emph{solution} to \eqref{LP} if $p^* = \val\left\langle c,x^*\right\rangle$, {\it i.e.} if the infimum in \eqref{LP} is attained at $x^*$.
