...
$\val$ is a valuation on $K$. The prototype of valued field we target is the field $K=\QQ_p$ of $p$-adic numbers, equipped with
its $p$-adic valuation. A polyhedron over a valued field is naturally defined as the set of vectors for which certain
...
%%%\footnote{\simone{On utilise vraiment le ``complete'' et ``discrete'' ici ? en tous cas pas pour la définition. Peut être dire (après la définition) qu'on fait cette hypothèse même si certains résultats sont vrais pour une valuation plus générale ?}} 
%We end by presenting an algorithm for solving linear optimization problems over $K$.
...
%%means the entries of $v \in K^n$ have nonnegative valuation.
...
%%% VOIR SI NECESSAIRE:
%%%Without loss of generality\footnote{je ne sais plus si ça est nécessaire, si ce ne l'est pas, je l'enleverais.}, one may assume in \Cref{def_polyhedra} that for every $i=1,\ldots,d$ there exists $x \in P$ such that $\val(\ell_i(x))<+\infty$.
...
\begin{example}
  ...
  %%%%%(its affine hull is indeed the whole $K^n$).
\end{example}
...
with $\PP_I \coloneq \left\lbrace x \in K^n,  Ax+v \geq 0 \right\rbrace$
