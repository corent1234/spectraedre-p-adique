...
$\PP = P_A^{-1}(\PP_2)$ for some polyhedron $\PP' \subset K^n$
...
and $\PP_E \coloneq \left\lbrace x \in K^n,  Cx+D=0 \right\rbrace = v + \overrightarrow{\PP_E}.$
\footnote{c'est quoi le overrightarrow sur $\PP_E$ ?}
Then\footnote{je comprends pas la notation $f_{\PP_E}$.}
$f(\PP) = f_{\PP_E}(\PP_I).$
...
\begin{corollary} \label{cor:caracdim1}
   If $\PP$ is a polyhedron in $K$ then $\PP$ is either empty or a ball (possibly with infinite radius).
\end{corollary}

\begin{proof}
    Let $\PP$ be such polyhedron. We assume that $\PP \neq \emptyset$. By \ref{prop:carac} there exist a polydisc $D = \{x \in K^n : \val s_i x_i - b_i \ge 0, i=1,\ldots,n \}$ for some $s_1,\ldots,s_n, b_1,\ldots,b_n \in K ^n$ and an affine map $f : K^n \to K$ such that $\PP = f(D)$. Let $a_0,a_1,\ldots,a_n \in K$ such that $f(x) = a_0 + a_1 x_1 + \ldots + a_n x_n$. We can assume, without loss of generality, that the $a_i$'s are nonzero. 
