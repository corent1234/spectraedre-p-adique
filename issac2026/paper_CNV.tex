...
Below we often use\footnote{rajouté notation $\succeq$ et modifié dans le texte}
the following matrix form (according to notation of
\Cref{def_polyhedra}) : let $A \in K^{d \times n}$, $B \in K^d$, $C \in K^{e \times n}$, and
$D \in K^e$ be the matrices such that $Ax+B = (\ell_1,\ldots,\ell_d)^T$
and $Cx+D = (m_1,\ldots,m_e)^T$.
Then the polyhedron in \eqref{eq_def_polyhedra} is compactly defined as
\begin{equation}\label{eq_def_polyhedra_compact}
  \PP = \left\lbrace x \in K^n : Ax+B \succeq 0, Cx+D = 0 \right\rbrace,
\end{equation}
where $v \succeq 0$ means the entries of $v \in K^n$ have nonnegative valuation.
...
\end{lemma}
\begin{proof}
  Assume $\PP$ is defined in matrix form as in \eqref{eq_def_polyhedra_compact}\footnote{j'ai rajouté
  avant cette notation}, and let $U$ be the matrix of
...
\begin{proof}
  ...
  with $A \in M_{d,n}(K)$, $B \in K^d$,
  and $x=(x_1,\dots,x_n)$ a column vector. 

We can compute a Smith Normal Form SNF($A'$)
of the $q \times (n-1)$ matrix $A'$ defined by the 
$n-1$ first columns of $A$
\[
\mathrm{SNF}(A') = \diag(\delta_1,\ldots,\delta_r,{\bf 0}_{q-r,n-1-r})
\]
...
$f(\PP_2)$ is a polyhedron for $\PP_2$
...
and we are reduced to $\PP_2$ defined by the linear matrix inequality:
\[
\begin{bmatrix}
...
\end{proposition}
\begin{proof}
  We identify $f$ with its matrix in $\allmat_{n,m}(K)$, and we
...
and
$$
\Delta = \diag(\lambda_1, \ldots, \lambda_r, {\bf 0}_{\min(m,n)-r}) \in \mathrm{SNF}(f)\in \allmat_{n,m}(K)
$$
