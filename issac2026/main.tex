...
with $\pi \in K$ a uniformizer. The valuation ring of $K$ is
$$
\OK = \{x \in K : \val(x) \geq 0\},
$$
$\kappa = \OK/\mathfrak{m}_K$ its residue field, with
$\mathfrak{m}_K = \{x \in K : \val(x) > 0\}$. An element of $\OK$ is called integral (at $\val$).
...
size $p \times q$ by ${\bf 0}_{p,q}$, and the identity matrix of size $t \times t$ by $I_t$.
