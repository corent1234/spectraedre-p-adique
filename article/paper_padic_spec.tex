...
The last proposition allows to only consider the resolution of the \ref{DLP} problem.
...
\begin{proof}
  ...
  It follows that $\val \chi_M (\rho) = \val \left(\rho^d+ \sum_{i=0}^{d-1} a_i \rho^i \right) = + \infty$ and $\val \rho^d = d \val \rho <0$.
  The properties of the non-archimedean inequality followed by $\val $ implie that $\val \left( \sum_{i=0}^{d-1} a_i \rho^i\right) = \val \rho^d$ and $\val \left( \sum_{i=0}^{d-1} a_i \rho^i\right) \ge \min \limits_{i=0,..., d-1} \val \left(a_i \rho^i\right)$. Therefore, there exists $i \in \left\{0,...,d-1\right\}$ such that $\val( a_i \rho^i) \le \val\rho^d$ {\it i.e.} such that $\val (a_i) \le (d-i) \val \rho <0$. That is all the results we need to prove the result.
...
\begin{proof}
Let $A$ be the annuli defined by $a<b \in R^*_+$. We consider for all $x,y \in K $ the spectrahedron $\mathcal{S}= \left\{(x,y) \in K  : M(x,y) := \pi^ax \oplus \pi^{-b}y \oplus \begin{bmatrix} \pi^{-1} & -\pi^{-1}x \\ \pi^{-1}y & - \pi^{-1}  \\ \end{bmatrix} \succeq 0\right\}$.
