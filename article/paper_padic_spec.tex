...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
      ...
      \IF{$\bigwedge_{i=r+1}^d \val(b_{i}) \geq 0$}
        ...
        \RETURN [UNBOUND, []] \hfill {\color{olive} \# \Cref{prop:reduc}, item \ref{prop:reduc_it2}}
        \ELSE
          ...
          \IF{$\val \lambda < v$}
            ...
            \RETURN [FEAS, $y$]
          \ELSE
          \STATE $i \leftarrow \min_{1 \leq i \leq r} \{i : v = \val (c_i/s_i)\}$
...
\begin{lemma}\label{solutions}
  Assume \eqref{LP} with data $(S,b',c',{\bf 0}, 0)$ be as in \Cref{solsLP} is feasible,
  with feasible set $\PP' = \{y \in K^n : Sy+b' \geq 0\}$.
  Let $\lambda = -\sum_{i=1}^r c'_i b'_i/s_i$ and $v = \min_{1\le i\le r} \val (c'_i/s_i)$.
  If $\val \lambda < v$, the minimum is $\val \lambda$ and is attained on every $x \in \mathcal{P}'$;
  if $\val \lambda \geq v$, the minimum is $v$ and is attained on ${\bf e}_i = (0,\ldots,0,1,0,\ldots,0}$.
\end{lemma}
\begin{proof}
