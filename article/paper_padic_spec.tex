...
\begin{abstract}
  \noindent
  Real polyhedra and real spectrahedra are respectively the feasible sets of linear programming
  (LP) and semidefinite programming (SDP). This paper investigates versions of these sets and of
...
\begin{definition}
  We say that a matrix $M \in \allmat_{d,d}(K)$ is \emph{positive semidefinite} ($M \succeq 0$)
...
Recall that the {\it Newton polygon} $\mathcal{P}$ of a polynomial $P = \sum_{i=0}^{n} a_i T^i \in K[T]$ is
defined as the inferior convex hull of the set $\{(i, \val a_i) \,|\, i\in \{0,...,d\} \}$, {\it i.e.} the
graph of the greatest convex function $\mathcal{P} : [0,n] \to \R$ such that $\mathcal{P}(i) \leq \val a_i$
for $i=1,...,n$. This function is piecewise affine and its slopes are exactly the opposites of the valuations
of the roots of $P$.
  %%%These are all the results we will need for this proof.

Testing positive semidefiniteness reduces to checking the coefficients of $\chi_M$.
\begin{theorem}
  \label{caracsdp}
...
\begin{proof}
  ...
  $$
  \min_{0 \leq i \leq d-1} \val \left(a_i \rho^i\right) \leq \val\left(\sum_{i=0}^{d-1} a_i \rho^i\right) = \val \rho^d
  $$
  ...
  Suppose now $\chi_M \not\in \OK[T]$. There exists $j \in \{0,...,d-1\}$ such that $\val a_j <0$. The polynomial $\chi_M$ being monic, the point with abscissa $n$ of the Newton polygon of $\chi_M$ is $(n,0)$. 
