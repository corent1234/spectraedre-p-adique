...
\begin{abstract}
  ...
  of linear programming (LP) and semidefinite programming (SDP) and form the family of
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  for some $d, e \in \mathbb{N}$. Let $T : K^{\eta} \to K^n$ be a surjective linear map such that
  $\text{im}(T) = M$, with $\eta = \dim(M)$.
  Let $y = (y_1, \ldots, y_\eta)^T \in K^{\eta}$ be variables on $M$, so that $x \in M$ iff
  $x = Ty$ for some $y \in K^{\eta}$. Let $A_0,A_1,\ldots,A_n \in K^{d \times d}$
  be such that
  $$
  ...
  One has that
  $$
  P = L \cap M = \{x \in K^n : x = Ty \in M, A_0+\sum_{i=1}^n x_iA_i \succeq 0\}.
  $$
%%% OLD PROOF
...
\begin{definition}[Annulus]\label{def_annuli}
  A set of the form $A = \{x \in K \,|\, a \le \val(x) \le b\}$, for $a,b \in \R^*_+$,
  is called \emph{annulus}.
\end{definition}

\begin{definition}[Semidefinite-representable set]\label{def_annuli}
  ...
  $$
  that is $S$ satisfies $x \in S$ if and only if there exists $y \in K^m$ such that
  $(x,y) \in S_M \subset K^{n+m}$, for $M(x,y) = A_0+\sum_{i=1}^n x_iA_i + \sum_{j=1}^m y_jB_j$.
  The integer $m$ is called the height of the representation.
\end{definition}

Of course spectrahedra are SDR sets with height $m=0$.
In order to distinguish between spectrahedra and possibly non-spectrahedral semidefinite-representable sets
we usually call \emph{spectrahedral shadows} the SDR sets with height $m>0$.

\begin{theorem}
Annuli are spectrahedral shadows with height $m=1$.
\end{theorem}
\begin{proof}
  Let $A$ be as in \Cref{def_annuli}, for some $a,b \in \R^*_+$, assume $a<b$. Consider the spectrahedron
  $S_M \subset K^2$ defined by
  $$
  S_M= \left\{(x,y) \in K^2 : M(x,y) := \diag\left(\pi^ax, \pi^{-b}y, \begin{bmatrix} \pi^{-1} & -\pi^{-1}x \\ \pi^{-1}y & - \pi^{-1}  \\ \end{bmatrix}\right) \succeq 0\right\}.
  $$
We claim that $x \in A$ if and only if there exists $y \in K$ such that $(x,y) \in S_M$.
