...
We further assume that $\PP \neq \emptyset$, and we identify $f$ with its matrix in $K^{n \times n}$.

Thanks to Definition \ref{smith_nf}, (the matrix of) $f$
can be written $f=Q^{-1} \Delta P$
with $P \in \GL_n(\OK),$ $Q \in \GL_m(\OK)$
and $\Delta = SNF(f)\in K^{m \times n}$ of the form $\diag(\lambda_1, \ldots, \lambda_r, {\bf 0}_{\min(m,n)-r})$
...
\end{array}
\end{equation}
with $c \in K^n$ a fixed vector of size $n$ that represents the cost, $A \in K^{m \times n}$, $b \in K^m$,
...
invertible in $\OK$, thus they preserve inequalities: indeed, if $z \in K^n$ and $M \in \OK^{n \times n}$ such
...
invertible in $\OK$. Therefore, for all matrix $M \in K^{n \times n}$ with $S=SNF(M)$ and for all $z \in K^n$,
...
\begin{proposition} \label{solsLP}
  Let $A \in K^{m\times n}, b \in K^m, c\in K^n$ and let $S = SNF(A) = QAP^{-1} \in K^{m \times n}$, with
...
\begin{proposition}\label{prop:reduc}
  Let $(A,b,c), (S,b',c') \in K^{m \times n} \times K^m \times K^n$ be as in \Cref{solsLP}, where
  $S=SNF(A)$. Then
  \begin{enumerate}
...
\begin{proposition}
  ...
  The set $K^{d \times d}_+$ is both open and closed.
\end{proposition}
\begin{proof}
...
Furthermore, $K^{d\times d}_+ = \chi^{-1}(\OK[X]) $ where $\chi$ is the function that maps a matrix to its characteristic polynomial which is continuous.
\end{proof}
...
\begin{proposition}
  The set $K^{d \times d}_+$ is a cone.
\end{proposition}
\begin{proof}
  The result is immediate following the definiton of $K^{d \times d}_+$.
\end{proof}
...
\begin{definition}
  ...
  $\mathcal{S} = \mathcal{L} \cap K^{d\times d}_+$,
...
\begin{remark}
  ...
  $S = \varphi^{-1}(K^{d \times d}_+) = \{x \in K^n : A_0+x_1A_1+\cdots+x_nA_n \succeq 0\}$
  of $K^{d \times d}_+$ under the map $\varphi(x) = A_0+x_1A_1+\cdots+x_nA_n$, is a \emph{spectrahedron
  in $K^n$}.
\end{remark}
...
\begin{remark}
  ...
  one has $(\mathcal{L}_1 \cap K^{d \times d}_+) \cap (\mathcal{L}_2 \cap K^{d \times d}_+) =
  (\mathcal{L}_1 \cap \mathcal{L}_2) \cap K^{d \times d}_+$. Now
