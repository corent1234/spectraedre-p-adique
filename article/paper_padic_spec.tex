...
\begin{remark}
  ...
  of $K^{d \times d}_+$ under the map $\varphi_A : K^n \to K^{d \times d}$, defined as
  $\varphi_A(x) = A_0+x_1A_1+\cdots+x_nA_n$, is a \emph{spectrahedron
  in $K^n$}. Remark that the map $\varphi_A$ is in general not injective, unless $A_1,\ldots,A_n$ are linearly
  indepedent.
\end{remark}
...
\begin{proposition}
  A polyhedron in $K^{d \times d}$ is a spectrahedron in $K^{d \times d}$.
\end{proposition}
\begin{proof}
