...
\end{corollary}
\begin{proof}
$\OK$ is a ring therefore the characteristic polynomial of matrix whose coefficients are $p$-adic integers have coefficients ins $Pp$ \simone{je comprends pas}. We conclude by \Cref{caracsdp}.
\end{proof}

\begin{remark}
  The reverse inclusion of \Cref{cor_caracsdp} is false. For instance the characteristic polynomial of
...
Remark that \Cref{def_spectrahedra} is invariant under intersection: the intersection of two
...
\begin{proof}
  ...
  be the polyhedron given in \Cref{def_polyhedra}, for $Q = \{x \in K^n : \val(\ell_i) \geq 0, i=1,\ldots,d\}$
