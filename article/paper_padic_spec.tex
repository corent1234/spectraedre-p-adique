...
\begin{definition}[Smith Normal Form] \label{smith_nf}
  ...
  The matrix $S$ is called the \emph{Smith Normal Form (SNF)} of $M$, and the elements $p^{a_i}$ the
  \emph{invariant factors}.
\end{definition}

\begin{remark}
  The valuation of the first coefficient\footnote{invariant factor?} of the SNF of a matrix
  $M \in \allmat_{n,n} \left( \QQ_p \right) $ is the minimum of the valuations of the matrix
  $M$'s terms (\simone{proof or reference}). In particular, the the SNF of matrix of $\mathcal{M}_n \left( \OK \right) $ have coefficients in $\ZZ_p$ .
 The $r = \text{rank} M$ first diagonals coefficients of $S$ are exactly its non-zero coefficient.
\end{remark}
