...
\begin{algorithm}
    \caption{SOLVELP($A,b,c,D,e$)}\label{algo_LP}
    \begin{algorithmic}[1]
      \REQUIRE
      ...
      \ENDIF
      \IF{$D \neq {\bf 0}$}
      \STATE $[\mathrm{STATUS}, y] \leftarrow $
      \RETURN 
      \ENDIF
      %\FOR{epochs = $1$ to $N$}
      %\WHILE{$(j\le m)$}
...
\begin{proof}
  ...
  Next, assume $D={\bf 0}$, then $Dx=e$ and $Ax + b \le 0$ if and only if
...
\begin{proof}
  ...
  Next, for all $c \in K^n$, we have $\langle c, x\rangle = \langle c, P^{-1}Px \rangle =
  \langle \left(P^{-1}\right)^T c, Px \rangle = \langle c',y\rangle$, which proves
  \Cref{solsLP_item2}.
\end{proof}
