...
\begin{proof}
  ...
  First suppose $M \not \in \allmat_{d,d}^+(K)$, thus $\chi_M$ has a root $\rho$ with $\val\,\rho<0$.
  It follows that
  $$
...
\begin{example}
  Let
  $$
  M = \begin{pmatrix} 5 + \frac{3}{5} & \frac{4}{5} \\ \frac{4}{5} & -\frac{3}{5} \end{pmatrix}.
  $$
  The characteristic polynomial of $M$
  is $\chi_M = T^2-5T-4$. However when seen as a matrix over $K=\mathbb{Q}_5$, then $\chi_M \in \mathbb{Z}_5[T] = \OK[T]$: then $M \in \allmat_{2,2}^+(\mathbb{Q}_5)$, nevertheless $M \not\in \allmat_{2,2}(\mathbb{Z}_5)$, hence $\allmat_{2,2}^+(\mathbb{Q}_5) \not\subset \allmat_{2,2}(\mathbb{Z}_5)$.
\end{example}
