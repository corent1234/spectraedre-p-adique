...
Let $\sym_d(\R)$ be the vector space of $d \times d$ real symmetric matrices. A matrix $M \in \sym_d(\R)$
...
spectrahedra, where the defining matrices $A_0, \ldots, A_n$ can be chosen diagonal. Thus SDP is a generalization
...
\begin{definition}
  Let $\ell_1,\ldots,\ell_d \in K[X]_1$ be linear inequalities on $X = (X_1, \ldots, X_n)$.
  A {\it polyhedron} is a subset $P \subset K^s$ defined by linear inequalities:
  $P = \val\left( \ell_1(x)\right)\ge 0$,\ldots, $ \val\left( \ell_n\left( x \right) \right)\ge 0$ for $ \ell_1,\ldots, \ell_n $ fixed linear forms and for all $x \in K ^s$.
\end{definition}
