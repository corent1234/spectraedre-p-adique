...
In this section we present \Cref{algo_LP}, \textbf{SOLVELP}, for solving Problem \eqref{LP}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
      ...
      \RETURN $\textbf{SOLVELP}(AJ,b+A x_0,J^T c, {\bf 0},0)$ \hfill {\color{olive} \# \Cref{reducD}}
      \ENDIF
      \IF{$A$ not in Smith Normal Form}
      \STATE $[Q,S,P] \leftarrow \textbf{SNF}(A)$
      \STATE $[\mathrm{STATUS}, y] \leftarrow \textbf{SOLVELP}(S,Qb,(P^{-1})^Tc,{\bf 0},0)$
      \RETURN $[\mathrm{STATUS}, P^{-1}y]$ \hfill {\color{olive} \# \Cref{solsLP}}
      \ELSE
      \IF{$\bigwedge_{i=r+1}^d \val(b_{i}) \geq 0$}
      \STATE STATUS $\leftarrow$ FEAS
      \STATE give solution in this case
      \ELSE
      \STATE
      \STATE
