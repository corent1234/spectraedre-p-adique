...
The \emph{dimension} of $P$ is defined as the dimension of $\aff{P}$ (for $e=0$,
$\aff{P}=K^n$ and $P$ has dimension $n$).
The \emph{relative interior} of $P$ is defined as $\relint{P} = \{x \in P : \val(\ell_1 \oplus \cdots \oplus \ell_d)
< +\infty\}$ (\simone{pas clair si besoin de dimension et relint}).
...
\end{corollary}
\begin{proof}
$\OK$ is a ring therefore the characteristic polynomial of matrix whose coefficients are $p$-adic integers have coefficients ins $Pp$ \simone{je comprends pas la fin de la phrase}. We conclude by \Cref{caracsdp}.
\end{proof}
...
\begin{definition}
  ...
  Let $\mathcal{L} = A_0+\span{A_1,\ldots,A_n} \subset \mathcal{M}_n(K)$ be an affine space, for some
  $A_0,A_1,\ldots,A_n \in \mathcal{M}_n(K)$.
  The set $\mathcal{L} \cap \mathcal{M}_n^+(K)$ is called a \emph{spectrahedron}. Its 
  its canonical preimage $S_A = \{x \in K^n : A_0+x_1A_1+\cdots+x_nA_n \succeq 0\}$
  is also called spectrahedron.
\end{definition}

\begin{remark}
  The class of spectrahedra is closed under intersection. Indeed, for affine spaces $\mathcal{L}_1,\mathcal{L}_2$
  one has $(\mathcal{L}_1 \cap \mathcal{M}_n^+(K)) \cap (\mathcal{L}_2 \cap \mathcal{M}_n^+(K)) =
  (\mathcal{L}_1\cap\mathcal{L}_2) \cap \mathcal{M}_n^+(K)$. Now
