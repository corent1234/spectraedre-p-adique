...
We formalize the algorithm described previously in \Cref{algo_LP}:
...
\begin{algorithm}
    ...
    \begin{algorithmic}
      ...
      \ENSURE A feasibility type (INFEAS, FEAS) and a solution (if FEAS)
      \vspace{0.1cm}
      \hline
      \vspace{0.1cm}
      \STATE $\mathrm{SOLVELP} (f_i,w_i,o_j)$
      \FOR{epochs = $1$ to $N$}
      \WHILE{$(j\le m)$}
