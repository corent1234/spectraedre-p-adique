...
and $\left\langle c, x \right\rangle := c_1 x_1+\cdots +c_n x_n$. When $A$ is a diagonal matrix and $D = 0$\footnote{\simone{pourquoi la contrainte $D=0$ ?}}, we often refer to \eqref{LP} as a \emph{diagonal} linear program.
...
linear program to a diagonal linear program. The reduced problem is then easily solved
by simple calulations in $K$\footnote{\simone{changer formulation}}.
...
Concerning the usage of SNF in the resolution of \eqref{LP}, note that the transition matrices are invertible in $\OK$ and as such they preserve inequalities.
...
Therefore, for all matrix $M \in K^{n\times n}$ with $S = \mathrm{SNF}(M)$
and for all $z \in K^n$,
...
\begin{proposition} \label{solsLP}
  Let $A \in \allmat_{d,n}(K), b \in K^d, c\in K^n$ and let $S = \mathrm{SNF}(A) = QAP^{-1} \in \allmat_{d,n}(K)$, with
