...
\begin{proof}
  ...
  $K^n$, and let $y = (y_1, \ldots, y_\eta)^T \in K^{\eta}$ be variables on $M$, with $Ty = x$
  for some $T \in K^{n \times \eta}$ with $\text{rank}(T) = \eta$. Let $A_0,A_1,\ldots,A_n \in K^{d \times d}$
  be such that
  $$
